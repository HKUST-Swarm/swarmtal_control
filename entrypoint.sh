...
if [ "$1" == "launch" ]; then
    ...
    nice --20 ros2 run mavros mavros_node --ros-args -p fcu_url:=$FCU_URL \
     -r __ns:=/uav$DRONE_ID/mavros &
    ...
    ros2 run drone_commander drone_cmd.py ${@:2} -d $DRONE_ID
else if [ "$1" == "status" ]; then
    ros2 run drone_commander drone_status.py $VO_TOPIC
else if [ "$1" == "plot" ]; then
    ros2 run drone_commander swarmtal_csv_parser.py ${@:2}
else if [ "$1" == "test" ]; then
